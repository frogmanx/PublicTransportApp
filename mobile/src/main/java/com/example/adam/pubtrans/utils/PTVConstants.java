package com.example.adam.pubtrans.utils;

/**
 * Created by Adam on 31/05/2015.
 */
public class PTVConstants {
    public final static String TRANSPORT_TYPE = "transportType";
    public final static String STOP_ID = "stopId";
    public final static String LIMIT = "limit";
    public final static String LINE_ID = "lineId";
    public final static String RUN_ID = "runId";
    public final static String TRAIN_TYPE = "train";
    public final static String TRAM_TYPE = "tram";
    public final static String BUS_TYPE = "bus";
    public final static String NIGHTRIDER_TYPE = "nightrider";
    public final static String JSON_NEARMERESULT = "jsonnearmeresult";
    public final static String JSON_VALUES = "jsonvalues";
    public final static String HEADER_TYPE = "headertype";
}
